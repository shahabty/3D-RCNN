...
if __name__ == '__main__':
  if mode == 'train':
    ...
    train_loader = DataLoader(train_dataset,batch_size=cfg['train']['batch_size'], collate_fn = collate_fn,num_workers=0, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=cfg['val']['batch_size'], collate_fn = collate_fn,num_workers=0, shuffle=True)
    ...
    trainer = get_trainer(train_loader = train_loader,val_loader = val_loader, model = model,loss_function = cfg['train']['loss'], optimizer = 'Adam')

  else:
    test_dataset = get_dataset(name = cfg['data']['dataset'],mode = 'test',data_path = cfg['data']['data_path'],device = device)
    test_loader = DataLoader(test_dataset,batch_size=cfg['test']['batch_size'], collate_fn = collate_fn,num_workers=0, shuffle=True)
    ...
    tester = get_tester(test_loader = test_loader,model = model,device = device)
    tester.run()
